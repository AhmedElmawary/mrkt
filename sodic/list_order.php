<?php
 ...
 $message = require_once("get_headers.php");

if (! empty($message)) die($message);
...
function list_order($order_object)
{

    if ( empty ($order_object)) return [
        "code"=> 428,
        "mesage"=>"no order provided"
    ];
	...
	$query = "SELECT orders.id AS order_id,is_paid,delivery_staff_id,total_price, 
		...
		WHERE ( orders.created_at )  >=  date_add(current_timestamp() , INTERVAL -5 DAY)
		
		AND city.id=1 
		
		AND orders.area_id=8
        ...
        AND states_per_order.order_id = $order_id 
		
		AND user.id=orders.user_id
		
		AND states_per_order.created_at=(SELECT MAX(created_at) FROM states_per_order WHERE order_id=orders.id) ORDER BY orders.created_at DESC ";
    ...
    if (empty($result))return [
        "code"=> 590,
        "mesage"=>"some thing went wrong"
    ];
   $row =   mysqli_fetch_assoc($result);
   if( empty ($row)) return ["code"=>418, "message"=>"No order found"];
   $order_id = $row['order_id'];
    ...
    if (empty($row["total_price_update_on"])) {
        ...
        $query_2 = "SELECT store_id,price FROM order_update WHERE order_id='" . $order_id . "' order by id desc ";
...
$request = json_decode(utf8_encode(file_get_contents("php://input")));

$output = json_encode(list_order($request));
